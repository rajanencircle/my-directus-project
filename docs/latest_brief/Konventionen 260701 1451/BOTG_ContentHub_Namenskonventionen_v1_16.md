...
Version 1.16 · 26-07-01 · Basis: Schema-Analyse Staging + excursions-Rebuild + vehicles-Build + Feld-Harmonisierung + Geo-Collections-Build + field_dictionary-Ausbau (BOTG + Karawane) + Regel-IDs (§7) + `caption-less-radio` + `status-subcollection`

> Änderungen ggü. v1.15: **§7.3** — neue Regel-ID **`status-subcollection`** registriert (besitzendes Dokument **Field-Configs §1.4**): 3-Werte-Status **active / archived / deleted** für **Stammdaten-/„sonstige“ Sub-Collections** (z. B. `booking_partners`) — abgegrenzt von `status_primarix` (§1.1, Produkt) und dem Geo-Lifecycle (§1.3). Soft-delete via `archive_field = status`, `archive_value = deleted` (= Directus-Löschaktion), `unarchive_value = active`, Default `active`. Eingeführt mit Field-Configs v1.4.

> Änderungen ggü. v1.14: **§7.3** — neue Regel-ID **`caption-less-radio`** registriert (besitzendes Dokument **Field-Configs §0**): Radios, deren **choice-Labels selbsterklärend** sind (Reveal-Muster `partner_visibility` / `booking_channel` / `operator_direct`), bleiben **caption-less** (`translations: null`) — die Bedeutung trägt die **Auswahl selbst** und/oder der **Section-Titel**; ein Radio, das eine Beschriftung braucht, bekommt sie weiterhin. Scoped, nicht blanket. Eingeführt mit der Field-Configs-Präzisierung (v1.3).

> Änderungen ggü. v1.13: **§7 (neu)** — **Regel-IDs & `convention_ref`**: jede konfigurationsrelevante Regel erhält eine stabile `kebab-case`-Regel-ID; Verweise nutzen **nur die ID**, nie Abschnittsnummern/Dokumentversionen (driften); zentrale **Regel-ID-Registry** (ID → Dokument → aktueller Abschnitt) über alle drei Konventionsdokumente; `field_dictionary.convention_ref` (CSV/Tags, später M2M auf `standards`). Eingeführt mit dem field_dictionary-Ausbau.

> Änderungen ggü. v1.12: §5 — **§5.4 (neu)**: Labels und Info-Texte werden **verbatim aus dem Brief-Excel** übernommen, inkl. exakter Groß-/Kleinschreibung **und Umlaute** (Schlüssel bleiben ASCII §1.1; Labels/Notes tragen echte Umlaute). Geo-Status-Enum-Werte festgehalten (`new`/`in review`/`active`/`archived`). Eingeführt mit dem Geo-Collections-Build.
> Änderungen ggü. v1.11: §4 — **Geo-Adressfelder** ergänzt; das City/Town-Feld ist M2O → `places` und heißt **`place`** (Singular der Ziel-Collection, Label „City / Town"), **nicht `town`** (`town` in rental_companies/rental_depots auf `place` angeglichen). Konfiguration s. Field-Configs §6.
> Änderungen ggü. v1.10: **Begleitdokumente** um `BOTG_ContentHub_Field-Configs_EN` ergänzt (Konfiguration wiederkehrender Felder); §4.6 — Querverweis ergänzt, dass die konkrete **Konfiguration** der Relations-Muster (interface/options/conditions) dort liegt (diese Konventionen besitzen nur die **Feldnamen**).
> Änderungen ggü. v1.9: §2 **C3 final aufgelöst** (war der letzte offene Präfix-Widerspruch): reine **Per-Produkt-Daten-/Kind-Tabellen** (Kalkulator-Dimensionen, Preis-/Zeitraum-Tabellen, Preiszellen) folgen ab sofort dem **Plural**-Produkt-Präfix — `vehicles_rental_zones`, `vehicles_price_periods`, `vehicles_rental_periods`, `vehicles_surcharges`, `vehicles_prices` (vgl. Ist-Stand `excursions_prices`, `tours_price_periods`). **Standalone-Lookups** bleiben **Singular**-Präfix (`vehicle_categories`, `rental_depot_categories`). **Legacy-Ausnahme:** `hotel_prices`, `price_dates`, `room_categories`, `tour_routes`, `cruise_price_dates` bleiben als Altbestand bestehen (nicht umbenannt). Eingeführt mit dem vehicles-Build (5a/5b/5c).
> Änderungen ggü. v1.8: §2 C5 / Schnell-Entscheidung / Supplementary — Translation-Collection-**Beispiele** von Singular auf **Plural** korrigiert (`excursions_descriptions_translations`, `<produkt>_price_infos_translations`), passend zur eigenen Kurzregel „Inhaltswort immer Plural" und zur Implementierung (cruises/tours/excursions/vehicles). `excursions` war der letzte Singular-Altfall und ist im DEV nachgezogen. (Die Feld-Alias-Beispiele waren bereits Plural.)
...
> **Begleitdokumente:** `BOTG_ContentHub_Navigation_EN` (Navigations-Hierarchie), `BOTG_ContentHub_Schema_Build_Spec_EN` (PK/FK-Typen, Relations-Wahl, Hidden-Standards, Deploy-Prozedur) und `BOTG_ContentHub_Field-Configs_EN` (Konfiguration wiederkehrender Felder: interface/options/conditions). Diese Konventionen besitzen **Key/Label**, die Build Spec besitzt **Typ/Struktur**, die Field Configurations besitzen die **Feld-Konfiguration**.
...
| C3 | Produktspezifische Kind-/Daten-Tabellen: **Produkt-Präfix im Plural** + Inhalt im Plural (Legacy-Singular grandfathered) | `vehicles_rental_zones`, `vehicles_price_periods`, `excursions_prices`, `tours_price_periods`; *Legacy:* `hotel_prices`, `price_dates` |
...
| C5 | Übersetzungstabellen: Suffix `_translations` (nie `_locale`); mehrere Felder dürfen sich eine Translation-Collection teilen. **Mehrere Translation-Collections je Collection → themen-qualifiziert (s. u.), nie nummeriert.** | `hotels_translations`, `vehicles_translations`, `excursions_descriptions_translations` |
...
| Liste, die zu **einem** Produkt gehört (Repeater/Child/**Daten-/Preistabelle**, inkl. `_translations` & Surcharges) | **Plural** | `excursions_surcharges`, `tours_occupancies`, `vehicles_rental_zones`, `vehicles_prices`, `excursions_descriptions_translations` |
...
> *Gelöst (v1.10):* Reine Daten-/Kind-Tabellen folgen jetzt **ebenfalls dem Plural**-Produkt-Präfix (`vehicles_rental_zones`, `vehicles_price_periods`, `vehicles_prices`). Die **Legacy-Singular**-Namen (`hotel_prices`, `tour_routes`, `cruise_price_dates`, `price_dates`, `room_categories`) bleiben als **Altbestand** bestehen und werden nicht umbenannt. Damit ist der letzte Präfix-Widerspruch dieser MD aufgelöst.

**Supplementary (übersetzbarer JSON-Repeater, keine Sub-Collection):** Frei wiederholbare Headline/Text-Blöcke werden **nicht** als eigene O2M-Collection modelliert, sondern als **JSON-Repeater-Spalte in der jeweiligen `_translations`-Tabelle**: `description_supplementary` → `<produkt>_descriptions_translations`, `price_info_supplementary` → `<produkt>_price_infos_translations` — eine JSON-Zeile je Sprache, Felder `headline`/`text`. Spart die separaten Supplementary-Sub-Collections (Trade-off s. Build Spec §3).
...
| Geo-Adressfelder | M2O = Singular der Ziel-Collection: `place`(→places), `country`, `state`, `region`, `location_tour32`. **Nie `town`.** | `place` (Label „City / Town") |
...
> **Konfiguration → Field Configurations:** Diese Konventionen legen nur die **Feldnamen** der Muster fest. Die konkrete Konfiguration (Radio-Choices, `_eq`-Conditions, M2M-Template, Junction-Verdrahtung, M2O-Template) steht in **`BOTG_ContentHub_Field-Configs_EN`** §3 (BOTG-Filter) / §4 (Buchung).
...
### 5.4 Verbatim aus dem Brief — Schreibweise & Umlaute

Labels und Info-Texte (Notes) werden **zeichengenau aus dem Brief-Excel** übernommen (Spalten F/G/H bzw. I) — inkl. exakter **Groß-/Kleinschreibung und Umlauten** (ä/ö/ü/ß). Während **Schlüssel** ASCII-only sind (§1.1, keine Umlaute), tragen **Labels und Notes echte Umlaute** — z. B. „Kürzel", „Länder", „o. ä.", „Maßgebliche". Eine ASCII-Ersetzung im Label (etwa „Kuerzel") ist falsch.

Auch **gespeicherte Enum-/Anzeigewerte** folgen dem Brief. Der **Geo-Status-Enum** lautet `new` → `in review` → `active` → `archived` — kleingeschrieben, „in review" mit Leerzeichen; die gespeicherten Werte müssen exakt den Migrations-/Import-Daten entsprechen. (Feld-Konfiguration s. Field-Configs §1.3.)
...
## 7. Regel-IDs & Verweise (`convention_ref`)

Felder werden vielfach nicht frei konfiguriert, sondern folgen einer in diesen Dokumenten festgelegten Regel (Status-Dropdown, Layout-Hierarchie, Relations-Display, Translations-Sprachfeld, Feldbreite …). Damit ein Feld auf „seine" Regel verweisen kann — heute im `field_dictionary` über die Spalte `convention_ref`, später als Relation auf die geplante `standards`-Collection — gilt:

### 7.1 Stabile Regel-IDs (Slugs)

Jede konfigurationsrelevante Regel erhält eine **stabile, sprechende Regel-ID** im `kebab-case` (z. B. `status-primarix-dropdown`, `tabs-rule`). Die ID ist **versions- und abschnittsunabhängig**: Sie ändert sich nicht, wenn die Regel im Dokument verschoben oder das Dokument neu versioniert wird.

### 7.2 Keine Abschnittsnummern als Verweis

Verweise (in `field_dictionary.convention_ref`, in DEV-Tickets, in der `standards`-Collection) nutzen **ausschließlich die Regel-ID** — **nie** Abschnittsnummern (`§3.1`) oder Dokumentversionen (`v1.13`). Diese driften und wären sofort veraltet.

### 7.3 Regel-ID-Registry

Diese Tabelle ist die **Single Source of Truth** für die Auflösung `Regel-ID → besitzendes Dokument → aktueller Abschnitt`. Verschiebt sich ein Abschnitt, wird **nur die Spalte „Abschnitt" hier** aktualisiert; die ID bleibt. Die Registry umfasst Regeln aus **allen** Konventionsdokumenten.

| Regel-ID | Dokument | Abschnitt (aktuell) | Kurzbeschreibung |
|---|---|---|---|
| `collection-prefix` | Namenskonventionen | §2 C3 | Per-Produkt-Child Plural-Präfix vs. Stammdaten-Lookup Singular |
| `junction-naming` | Namenskonventionen | §2 C4 | M2M-Junction `a_b`, gemeinsames Präfix nicht verdoppeln |
| `translations-collection-naming` | Namenskonventionen | §2 C5 | `_translations`, themen-qualifiziert, nie nummeriert |
| `surcharge-trio` | Namenskonventionen | §2 C9 | Surcharge-Collection-Trio (Items / Item-Übers. / Calc-Übers.) |
| `layout-hierarchy` | Namenskonventionen | §3 | `ui_ > tab_ > block_ > section_`, kein Raw-in-Raw, kein Accordion |
| `tabs-rule` | Namenskonventionen | §3 | `ui_tabs`/`tab_*` erst ab ≥ 2 Tabs |
| `field-width-default` | Namenskonventionen | §3 | Half default; Full nur Textarea/WYSIWYG/Repeater/Gallery |
| `name-primary-field` | Namenskonventionen | §4.1 | Primäres Namensfeld `name_<entity>` |
| `engb-spelling` | Namenskonventionen | §1.7 | en-GB-Schreibvarianten in Schlüsseln (`colour`/`programme`/…) |
| `labels-verbatim` | Namenskonventionen | §5.4 | Labels/Notes zeichengenau aus Brief, inkl. Umlaute |
| `status-primarix-dropdown` | Field-Configs | §1.1 | `status_primarix` 4-Werte-Dropdown + Farben |
| `status-subcollection` | Field-Configs | §1.4 | active/archived/deleted für Stammdaten-/„sonstige“ Sub-Collections; archive_value=deleted |
| `partner-visibility-pattern` | Field-Configs | §3 | all/selected-Radio + `partner_selected`-M2M |
| `caption-less-radio` | Field-Configs | §0 | Selbsterklärende Radios bleiben label-los (`translations: null`); Info trägt choices und/oder Section-Titel |
| `relational-display-template` | Field-Configs / Build Spec | FC §4.6 / BS §3.1–3.2 | `{{<fk>.translations.name}}`, `related-values`, `enableCreate:false` |
| `translations-language-field` | Field-Configs | (neu, zu ergänzen) | Translations-Alias braucht `options.languageField:"code"` (sonst speichert der Inhalt nicht) |
| `menu-icon-colours` | Schema Build Spec | §6 | Produkt blau `#0DA4DE`, sichtbar-nicht-Produkt grau `#6B7785`, technisch uncoloured |

*(Registry wird mit jeder neuen Regel ergänzt; vergebene IDs sind permanent.)*

### 7.4 Geltung / Pflege

**Jede neu eingeführte Regel** in einem der drei Konventionsdokumente erhält **sofort eine Regel-ID** und wird in 7.3 registriert. Im `field_dictionary` tragen **alle regelgebundenen Felder** die passende(n) Regel-ID(s) in `convention_ref` (CSV/Tags, 0…n; leer bei regelfreien Feldern). Sobald die `standards`-Collection existiert, wird `convention_ref` von CSV auf eine **M2M-Relation** zur `standards`-Collection (PK = Regel-ID) umgestellt — die IDs bleiben identisch (vorwärtskompatibel).

### 7.5 Scope

Erfasst werden **Namens-/Layout-Regeln** (diese Konventionen), **Konfigurations-Regeln** (Field-Configs) und **Struktur-/Deploy-Regeln** (Build Spec) — gleichberechtigt in einer Registry.
