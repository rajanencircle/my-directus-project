module.exports = async function (data) {
  ...
  for (const item of items) {
    ...
    if (defaultMarginPreset && defaultMarginPreset.hotel_destinations) {
      marginMatchedException = defaultMarginPreset.hotel_destinations.find(
    ...
    for (const trans of item.translations || []) {
      ...
      if (marginMatchedException && marginMatchedException.translations) {
        ...
        defaultMarginPreset &&
        defaultMarginPreset.hotel_margin_translations
      ) {
        const pt = defaultMarginPreset.hotel_margin_translations.find(
          (t) => t.translations_id === localeId,
        );
        margin = pt ? pt.margin : 0;
      }

      // 2. Get Exchange Rate Value (Use existing rate from the translation)
