...
async function deleteCollections() {
  const isLoopMode = process.argv.some(
    (arg) => arg === "--loop" || arg === "-l",
  );
  let attempt = 1;
  ...
  try {
    while (true) {
      ...
      console.log(
        `Found ${nonSystemCollections.length} collections remaining.`,
      );
      ...
      for (const collectionName of nonSystemCollections) {
        ...
        if (deleteRes.status === 204) {
          ...
          console.log(
            `❌ (${error.errors?.[0]?.message || deleteRes.statusText})`,
          );
        }
      }
      ...
      if (
        successfulDeletionsInThisPass === 0 &&
        nonSystemCollections.length > 0
      ) {
        console.log(
          "\n⚠️ Progress stalled: No collections could be deleted in this pass despite remaining items.",
        );
        console.log(
          "This usually indicates a database-level constraint that needs manual intervention.",
        );
        break;
      }
