  ...
  <v-dialog class="upload-modal-dialog" :model-value="true" @update:model-value="(v: boolean) => !v && handleClose()" persistent>
    <v-card class="upload-card">
      ...
      <v-card-text class="card-body">
        <div class="section">
          <label class="section-label">{{ lbl('uploadToFolder', 'Upload to folder') }}</label>
        ...
        >
          ...
          <p class="drop-primary">{{ lbl('uploadDropPrimary', 'Drag & drop files here') }}</p>
          <p class="drop-secondary">{{ lbl('uploadDropSecondary', 'or click to browse') }}</p>
          <input
            ref="fileInputRef"
            type="file"
            multiple
        ...
        <div v-if="fileItems.length > 0" class="selected-wrap">
          <div class="selected-header">
            <span class="selected-label">{{ lbl('uploadSelectedLabel', 'Selected') }}</span>
            <span class="selected-sub">{{ pendingCount }} {{ lbl('uploadReadyCount', 'ready to upload') }}</span>
            <div class="spacer" />
      ...
      >
        ...
        {{ lbl('uploadGeoRequiredError', 'Please fill in required geography fields') }}: {{ missingGeoLevels.map((l) => l.label).join(', ') }}
      </div>
