<template>
  <div class="room-prices-table">
    ...
    <div v-else>
      <!-- Context Selector for Hotel view -->
      <div
        v-if="
...
export default defineComponent({
  props: {
    ...
    // Junction collection
    junctionCollection: { type: String, default: "hotels_translations_1" },
    junctionParentKeyField: { type: String, default: "parent_id" },
    ...
    translationsCollection: {
      type: String,
      default: "room_prices_translations",
    },
    translationsFKField: { type: String, default: "room_prices_id" },
  ...
  setup(props, { emit }) {
    ...
    // Translation options for Hotel view
    ...
    // Resolve parent ID and language ID from the current context
    const fetchTranslationInfo = async () => {
      ...
      if (props.collection === props.parentCollection) {
        ...
        try {
          ...
          console.error(
            "[RoomPricesTable] Error fetching junction records:",
            err,
          );
        }
        return;
      ...
      if (props.collection === props.junctionCollection) {
        if (props.values?.[props.junctionParentKeyField]) {
          ...
          if (hId) {
            ...
            if (
              ...
              // Continue to API fetch below to fill missing language info
            } else {
              return;
            }
          }
        ...
        try {
          const parentField =
            props.parentKeyField || props.junctionParentKeyField || "hotels_id";
          ...
          console.error(
            "[RoomPricesTable] Error fetching junction record:",
            err,
          );
        }
      }
    ...
    const fetchCurrencySymbols = async (rateKey: string) => {
      try {
        ...
        console.error(
          "[RoomPricesTable] Error fetching currency symbols:",
          err,
        );
      }
    };
    ...
    const fetchHotelAndRates = async () => {
      ...
      try {
        ...
        parentRecord.value = hotelRes.data.data;
        ...
        if (
          ...
          try {
            ...
            if (props.occupancySourceMode === "junction") {
              console.error(
                "[RoomPricesTable] Error fetching occupancy junction collection:",
                junctionErr,
              );
            }
          }
        ...
        console.error("[RoomPricesTable] Error fetching parent record:", err);
      }
    };
    ...
    const fetchLookupData = async () => {
      ...
      try {
        ...
        console.error("[RoomPricesTable] Error fetching lookup data:", err);
      }
    };

    // Fetch per-language sell prices and merge into items
    const fetchPriceTranslations = async (priceIds: string[]) => {
      ...
      try {
        ...
        console.error(
          "[RoomPricesTable] Error fetching price translations:",
          err,
        );
      }
    };
    ...
    const fetchItems = async () => {
      ...
      try {
        ...
        console.error("[RoomPricesTable] Error fetching items:", err);
        items.value = [];
      } finally {
        loading.value = false;
      }
    ...
    const persistChanges = async () => {
      ...
      try {
        ...
        changedItems.forEach((item) => {
          ...
          // 1. Update price record if buy price changed
          ...
          // 2. Update/Create translation record if sell price changed
          if (
            orig &&
            orig[props.sellPriceField] !== item[props.sellPriceField]
          ) {
        ...
        console.error("[RoomPricesTable] Save error:", err);
    ...
    try {
      ...
      authStore = stores.useAuthStore ? stores.useAuthStore() : null;
    ...
    const connectWebSocket = async () => {
      ...
      ws.onerror = () => {
        console.warn("[RoomPricesTable] WS: connection error");
      };
    };
    ...
    onMounted(async () => {
      ...
      await Promise.all([fetchLookupData(), fetchHotelAndRates()]);
      await fetchItems();
      connectWebSocket();
    });
