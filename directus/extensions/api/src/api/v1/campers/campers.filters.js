...
export function buildListFilter({ publishing_status }) {
  const filter = { rental_type: { _eq: 'camper' } };
...
// campers has no equivalent status/publish window fields on media/depots — no deep
...
// campers' PK (id) is an integer, so a numeric path param is treated as the internal
...
// from source_updated_at (see campers.service.js) to keep the two consistent.
