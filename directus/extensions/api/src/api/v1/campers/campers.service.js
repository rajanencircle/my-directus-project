...
export async function listSlimCampers(
  { page, limit, offset, publishing_status },
  { services, database, getSchema },
) {
  const schema = await getSchema();
  const { ItemsService } = services;
  const campersService = new ItemsService(COLLECTION, { knex: database, schema });

  const filter = buildListFilter({ publishing_status });

  const [rawItems, countResult] = await Promise.all([
    campersService.readByQuery({
      fields: LIST_FIELDS,
      sort: buildSort(),
      limit,
      offset,
...
export async function listFullCampers(
  ...
  for (const item of rawItems) {
    try {
      const detail = await getCamperDetails({ id: item.id.toString() }, context);
      data.push(detail);
    } catch (e) {
      console.error(`Failed to fetch full detail for camper ${item.id}`, e);
    }
  }
...
export async function getCamperDetails({ id }, { services, database, getSchema }) {
  const schema = await getSchema();
  const { ItemsService } = services;
  const campersService = new ItemsService(COLLECTION, { knex: database, schema });
  ...
  const filter = { _and: [buildIdFilter(id), { rental_type: { _eq: 'camper' } }] };

  const items = await campersService.readByQuery({
    fields: buildDetailFields({ schema, rootCollection: ROOT_COLLECTION, relations: DETAIL_RELATIONS }),
    filter,
    limit: 1,
  });

  const camper = items?.[0] ?? null;
  if (!camper) {
    throw new AppError(`Camper not found: ${id}`, HTTP_STATUS.NOT_FOUND);
  }

  const rentalCompanyId = camper.rental_company?.id ?? null;

  const [prices, priceCalcRows, surchargeCalcRows] = await Promise.all([
    pricesService.readByQuery({ fields: PRICE_FIELDS, filter: { vehicle_id: { _eq: camper.id } }, limit: -1 }),
    priceCalculationService.readByQuery({ fields: PRICE_CALCULATION_FIELDS, filter: { vehicle_id: { _eq: camper.id } }, limit: 1 }),
    surchargeCalculationService.readByQuery({ fields: SURCHARGE_CALCULATION_FIELDS, filter: { vehicle_id: { _eq: camper.id } }, limit: 1 }),
  ]);
  ...
  const depotObjects = (camper.depots_selected ?? [])
  ...
  const enrichedCamper = await enrichExchangeRates(camperData, database);
  return enrichedCamper;
}
