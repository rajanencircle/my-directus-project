...
export function createToursController(context) {
  return {
    async index(req, res) {
      ...
      const { search, country, region, state, season, sort, updated_after, status } = req.query;

      const result = await listTours(
        { page, limit, offset, search, country, region, state, season, sort, updated_after, status },
        context,
      );
