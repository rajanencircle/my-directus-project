  ...
  setup(props) {
        ...
        if (props.occupancySourceMode === "junction" || props.occupancySourceMode === "auto") {
          try {
            ...
            if (props.occupancySourceMode === "junction") {
              console.error("[ToursPricesTable/Direct] Error fetching occupancy junction:", junctionErr);
            }
          }
        }
      } catch (err) {
        console.error("[ToursPricesTable/Direct] Error fetching parent record:", err);
      }
    };
    ...
    const fetchLookupData = async () => {
      ...
      try {
        ...
        console.error("[ToursPricesTable/Direct] Error fetching lookup data:", err);
      }
    };
    ...
    const fetchItems = async () => {
      ...
      try {
        ...
        console.error("[ToursPricesTable/Direct] Error fetching items:", err);
        items.value = [];
      } finally {
        loading.value = false;
      }
    ...
    const persistChanges = async () => {
      ...
      try {
        ...
        console.error("[ToursPricesTable/Direct] Save error:", err);
    ...
    const handleBuyPriceInput = (groupKey: string, rowId: string, colId: string, event: Event) => {
      ...
      if (cell?.id !== undefined || cell?._isNew) {
        cell[props.buyPriceField] = numValue;
    ...
    const connectWebSocket = async () => {
      ...
      ws.onerror = () => { console.warn("[ToursPricesTable/Direct] WS: connection error"); };
    };
