...
function normalizedCheckboxOptions(field: UploadExtraField): NormalizedCheckboxOption[] {
  ...
  return raw.map((o: CheckboxOptionInput, i: number) => normalizeCheckboxOption(o, i)).filter((x): x is NormalizedCheckboxOption => x != null)
}

function parseUploadExtraFields(input: any): UploadExtraField[] {
  if (!input) return []
  const raw = typeof input === 'string' ? (() => { try { return JSON.parse(input) } catch { return null } })() : input
...
const canUpload = computed(() => fileItems.value.some((f) => f.status === 'idle') && !isUploading.value)

function addFiles(rawFiles: FileList | File[]) {
  for (const file of Array.from(rawFiles)) {
    const previewUrl = file.type.startsWith('image/') ? URL.createObjectURL(file) : null
    fileItems.value.push({ file, previewUrl, status: 'idle', progress: 0, errorMessage: null, uploadedId: null })
  }
}
...
function uploadFileXhr(item: FileItem): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    xhr.upload.onprogress = (event) => {
      if (event.lengthComputable) item.progress = Math.round((event.loaded / event.total) * 90)
    }
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        item.status = 'error'
        try {
          item.errorMessage = JSON.parse(xhr.responseText)?.errors?.[0]?.message ?? `Upload failed (${xhr.status})`
        } catch {
    ...
    xhr.onerror = () => { item.status = 'error'; item.errorMessage = 'Network error during upload.'; resolve(null) }
    xhr.ontimeout = () => { item.status = 'error'; item.errorMessage = 'Upload timed out.'; resolve(null) }
...
async function startUpload() {
  ...
  const uploadedIds = (await Promise.all(pending.map(uploadFileXhr))).filter((id): id is string => Boolean(id))
...
function handleClose() {
  if (anySucceeded.value) {
    const ids = fileItems.value.filter((f) => f.status === 'done' && f.uploadedId).map((f) => String(f.uploadedId))
    emit('uploaded', ids)
  }
  emit('update:modelValue', false)
}
...
<template>
  <v-dialog :model-value="modelValue" @update:model-value="(v: boolean) => !v && handleClose()" persistent>
    <v-card class="upload-card">
      ...
      <v-card-title class="card-title">
        ...
        <div class="spacer" />
        <button class="close-btn" type="button" :disabled="isUploading" @click="handleClose">
      ...
      <v-card-text class="card-body">
        ...
        <div class="section">
          ...
          <FolderDropdown v-model="selectedFolder" :exclude-id="uploadAreaFolder ?? null" />
        </div>
        ...
        >
          ...
          <input ref="fileInputRef" type="file" multiple class="hidden-input" @change="onFileInputChange" />
        </div>
          ...
          <div class="selected-grid">
            ...
            >
              <div class="tile-thumb">
                <img v-if="item.previewUrl" :src="item.previewUrl" alt="" class="tile-img" />
...
.close-btn:hover:not(:disabled) { color: var(--theme--foreground); }
...
/* Selected grid */
.selected-wrap { display: flex; flex-direction: column; gap: 10px; }
...
.selected-sub { font-size: 12px; color: var(--theme--foreground-subdued); }
...
.tile.error { border-color: var(--theme--danger, #dc3545); }
...
.tile-remove:hover:not(:disabled) { background: rgba(0, 0, 0, 0.65); }
.tile-remove:disabled { opacity: 0.5; cursor: default; }
...
.tile-progress-bar.done { background: var(--theme--success, #28a745); }

.tile-meta { padding: 8px 10px; display: flex; flex-direction: column; gap: 4px; }
.tile-name { margin: 0; font-size: 12px; color: var(--theme--foreground); overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }
.tile-error { margin: 0; font-size: 11px; color: var(--theme--danger, #dc3545); }

/* Extra fields */
.extra-fields { display: flex; flex-direction: column; gap: 12px; }
...
.flag-item.disabled { opacity: 0.6; cursor: default; }

.flag-check { width: 16px; height: 16px; }

.flag-label { font-size: 13px; font-weight: 600; color: var(--theme--foreground); }
