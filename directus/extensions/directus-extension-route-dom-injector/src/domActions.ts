...
function hideLabelForField(fieldEl: HTMLElement): void {
  const selectors = ['.field-label', '.form-field-label', '.label', '.v-form-field-label', '.v-label', 'label'];
...
export function handleHideLabels(config: true | HideLabelsByField): void {
  if (config === true) {
    document.querySelectorAll<HTMLElement>('[data-field]').forEach(hideLabelForField);
  } else if (config?.fields) {
    config.fields.forEach(fieldName => {
      const fieldEl = document.querySelector<HTMLElement>(`[data-field="${fieldName}"]`);
...
export function handleRunScripts(scripts: Array<() => void>): void {
  console.log('[route-dom-injector] handleRunScripts called, count=', scripts.length);
  scripts.forEach((fn, i) => {
    try {
      console.log(`[route-dom-injector] running script[${i}]`);
      fn();
      console.log(`[route-dom-injector] script[${i}] done, matched elements:`, document.querySelectorAll('.field-label .field-name').length);
    } catch (err) {
