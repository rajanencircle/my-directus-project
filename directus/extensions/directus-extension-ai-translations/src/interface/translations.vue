<template>
	<div class="translations">
			...
			<div class="header-title-row">
				...
				>
					...
					<button
						...
						:class="{ 'ai-translate-btn--active': !aiTranslating && selection.length > 0 && !disabled }"
					...
					<div class="lang-box-main">
						<transition name="icon-morph" mode="out-in">
							<v-icon
								...
								class="lang-icon"
								:class="{
									'lang-icon--active': hasContent(sourceLanguage) || isDraft(sourceLanguage),
									'lang-icon--empty': !hasContent(sourceLanguage) && !isDraft(sourceLanguage),
									'lang-icon--spin': hasContent(sourceLanguage) || isDraft(sourceLanguage)
								}"
					...
					<div class="lang-box-main">
						<transition name="icon-morph" mode="out-in">
							<v-icon
								...
								class="lang-icon"
								:class="{
									'lang-icon--active': hasContent(targetLanguage) || isDraft(targetLanguage),
									'lang-icon--empty': !hasContent(targetLanguage) && !isDraft(targetLanguage),
									'lang-icon--spin': hasContent(targetLanguage) || isDraft(targetLanguage)
								}"
		...
		<!-- ─── Field rows ─────────────────────────────────────────────── -->
		<div v-if="sourceLanguage && (targetLanguage || !splitViewOn)" class="fields-container">
			<div
				v-for="f in translatableFields"
				:key="f.field"
			...
			>
				...
				<div class="field-input-col">
					...
					<v-form
						...
						:fields="[unlabeled(f)]"
						:model-value="{ [f.field]: getRow(sourceLanguage)?.[f.field] ?? null }"
						:primary-key="getPrimaryKey(sourceLanguage)"
						:disabled="disabled"
						class="inline-form"
						@update:model-value="(val) => setFieldValue(sourceLanguage, f.field, val[f.field] ?? null)"
					/>
					<p v-if="f.meta?.note" class="field-note" v-html="f.meta.note" />
				</div>

				<!-- Selection Checkbox -->
				<div class="field-ai-col" v-show="splitViewOn">
					<v-checkbox
						v-model="selection"
						:value="f.field"
				...
				>
					...
					<v-form
						...
						:fields="[unlabeled(f)]"
						:model-value="{ [f.field]: getRow(targetLanguage!)?.[f.field] ?? null }"
						:primary-key="getPrimaryKey(targetLanguage!)"
						:disabled="disabled || f.field in pendingTranslations"
						class="inline-form"
						@update:model-value="(val) => setFieldValue(targetLanguage!, f.field, val[f.field] ?? null)"
					/>
					<p v-if="f.meta?.note" class="field-note" v-html="f.meta.note" />

					<!-- Blue translated text below the field when pending -->
					<div v-if="f.field in pendingTranslations" class="pending-translation-text">
						<template v-if="Array.isArray(pendingTranslations[f.field])">
							<div v-for="(item, idx) in pendingTranslations[f.field]" :key="idx" class="pending-repeater-item">
		...
		<v-dialog :model-value="langToDelete !== null" @update:model-value="!$event && cancelDelete()" @esc="cancelDelete">
			<v-card>
...
const activeLanguagesCollection = computed(() => props.languagesCollection || 'translations');
const activeLanguageField = computed(() => props.languageIndicatorField || props.languageField || 'code');
const startLanguage = computed(() => {
	if (props.useCurrentUserLanguage || props.userLanguage) {
		return (userStore.currentUser as any)?.language ?? props.defaultLanguage ?? null;
	}

	return props.defaultLanguage ?? null;
});
const startSplitView = computed(() => props.defaultOpenSplitView ?? props.defaultSplitView ?? false);
...
const hasPending = computed(() => Object.keys(pendingTranslations.value).length > 0);
...
const resolvedTranslationsCollection = computed<string | null>(() => {
	...
	const relation =
		...
		relations.find((r: any) => r.meta?.one_collection === props.collection && r.meta?.one_field === props.field) ??
		relations[0];
...
watch(
	...
	async (collection) => {
		...
		try {
			...
			console.error('[ai-translations] Failed to load translation fields:', err);
		}
	},
	{ immediate: true }
);
...
const langFkField = computed<string>(() => {
	const fkField = allFields.value.find((f: any) =>
	...
	if (localItems.value.length > 0) {
		...
		for (const candidate of [
			'translations_id', 'translations_code',
			'languages_code', 'languages_id',
...
const langPkField = computed<string>(() => {
	const fields = fieldsStore.getFieldsForCollection(activeLanguagesCollection.value) ?? [];
...
const langCodeToFkValue = computed<Record<string, any>>(() => {
	...
	const fkFieldMeta = allFields.value.find((f: any) => f.field === langFkField.value);
	...
	for (const lang of availableLanguages.value) {
		...
		map[code] = fkToCodeField ? code : (lang[langPkField.value] ?? lang.id ?? code);
	}
	return map;
});
...
const translatableFields = computed(() => {
	const skip = new Set([
		...
		'translations_id', 'translations_code',
		'languages_code', 'languages_id',
		'id', 'sort', 'date_created', 'date_updated', 'user_created', 'user_updated',
	]);
...
function unlabeled(f: any) {
	return { ...f, name: null, meta: { ...f.meta, note: null } };
}
...
async function loadLanguages() {
	...
	try {
		const { data } = await api.get(`/items/${activeLanguagesCollection.value}`, {
			params: { limit: -1 },
		});
...
watch([activeLanguagesCollection, activeLanguageField], () => {
	void loadLanguages();
}, { immediate: true });
...
watch(
	...
	async (newVal) => {
		...
		if (isIds) {
			...
			if (col) {
				try {
					const pkField = allFields.value.find((f: any) => f.schema?.is_primary_key)?.field || 'id';
...
function openSelect(refName: 'source' | 'target') {
	...
	if (select && select.$el) {
		const btn = select.$el.querySelector('button.inline-display')
...
// AI Translation
// ─────────────────────────────────────────────────────────────────────────────
async function translateSingleField(fieldName: string, src: string, tgt: string): Promise<string | null> {
	const sourceText = getFieldValue(src, fieldName);
	if (!sourceText || typeof sourceText !== 'string' || !sourceText.trim()) return null;
...
async function runAiTranslateAll() {
	if (!sourceLanguage.value || !targetLanguage.value || selection.value.length === 0) return;
	...
	try {
		...
		const textInterfaces = new Set(['input', 'input-multiline', 'input-rich-text-html', 'input-rich-text-md', 'textarea']);
		const repeaterInterfaces = new Set(['list', 'repeat']);

		const batch: Record<string, string> = {};
		const repeaterFields: { fieldName: string; items: Record<string, any>[] }[] = [];

		for (const fieldName of selection.value) {
			const f = translatableFields.value.find((tf: any) => tf.field === fieldName);
			if (!f) continue;

			const val = getFieldValue(sourceLanguage.value!, f.field);

			// Handle repeater/list fields
			if (repeaterInterfaces.has(f.meta?.interface) && Array.isArray(val) && val.length > 0) {
		...
		for (const { fieldName, items } of repeaterFields) {
			...
			for (const item of items) {
				...
				for (const [key, val] of Object.entries(item)) {
					if (typeof val === 'string' && val.trim().length > 0 && key !== 'key') {
						const { data } = await api.post('/ai-translations/translate', {
							text: val,
							sourceLanguage: sourceLanguage.value,
							targetLanguage: targetLanguage.value,
...
watch(
	...
	(langs) => {
		...
		if (!targetLanguage.value && langs.length > 1) {
			targetLanguage.value = langs.find((l) => l[key] !== sourceLanguage.value)?.[key] ?? null;
		}
	},
	{ immediate: true }
);
...
.lang-box.target-lang {
	background-color: var(--theme--secondary-background, var(--theme--secondary-10, #fce4ec));
...
/* Pending state: light purple highlight */
.field-input-col.pending {
...
.pending-translation-text {
	...
	padding: 6px 10px;
	border-radius: 6px;
	border-left: 3px solid color-mix(in srgb, var(--ai-translate-purple) 70%, white);
}
