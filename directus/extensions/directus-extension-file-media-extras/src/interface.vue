...
const props = withDefaults(
  defineProps<{
    ...
    download_format_presets?: unknown;
  }>(),
  {
    value: null,
    disabled: false,
...
async function loadFileMeta() {
  ...
  try {
    ...
    fileMeta.value = d;
    fileType.value = d?.type ?? null;
    filenameDownload.value = d?.filename_download ?? null;
  } catch {
    // silent — non-critical
  }
}
...
function downloadOriginal() {
  const id = fileId.value;
  if (!id) return;
  triggerDownload(`/assets/${id}?download`, filenameDownload.value);
}
...
onMounted(async () => {
  if (fileId.value) {
    await Promise.all([loadFileMeta(), loadReverseLinks(fileId.value)]);
  }
});
</script>
...
<template>
  <div v-if="!fileId" class="empty-state">
    Save this record first to see downloads and usage.
  </div>
