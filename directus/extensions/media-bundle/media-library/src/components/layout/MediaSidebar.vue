    ...
    <template v-if="albumsStore.albums.length > 0 || albumsStore.isLoading">
      ...
      >
        ...
        <v-list-item-icon class="album-delete-btn" @click.stop="promptDeleteAlbum(album.id, album.name)">
          <v-icon v-tooltip="'Delete Album'" name="delete_outline" small />
        </v-list-item-icon>
      </v-list-item>
    </template>
  </v-list>
    ...
    <v-card>
      ...
      <v-card-text>
        <v-notice type="danger">
          <strong>{{ albumToDeleteName }}</strong> will be permanently deleted. This cannot be undone.
        </v-notice>
      </v-card-text>
      <v-card-actions>
        <v-button secondary @click="deleteDialogOpen = false">Cancel</v-button>
...
const albumsSectionLabel = computed(() => settings.value.albums_section_label || 'Albums')
