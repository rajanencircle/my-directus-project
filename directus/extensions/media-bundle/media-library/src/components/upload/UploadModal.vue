...
function uploadFileXhr(item: FileItem): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          item.status = "error";
          item.errorMessage = lbl('upload_err_response', 'Uploaded but failed to process response.');
        }
      } else {
        item.status = "error";
        try {
    ...
    xhr.onerror = () => {
      item.status = "error";
      item.errorMessage = lbl('upload_err_network', 'Network error during upload.');
      resolve(null);
    };
    xhr.ontimeout = () => {
      item.status = "error";
      item.errorMessage = lbl('upload_err_timeout', 'Upload timed out.');
      resolve(null);
    };
  ...
  >
    <v-card class="upload-card">
      ...
      <v-card-title class="card-title">
        ...
        {{ lbl('upload_modal_title', 'Upload Files') }}
      ...
      <v-card-text class="card-body">
        <!-- Folder picker -->
        <div class="section">
          <label class="section-label">{{ lbl('upload_to_folder', 'Upload to folder') }}</label>
          <FolderDropdown
        ...
        >
          ...
          <p class="drop-primary">{{ lbl('upload_drop_primary', 'Drag & drop files here') }}</p>
          <p class="drop-secondary">{{ lbl('upload_drop_secondary', 'or click to browse') }}</p>
          <input
            ref="fileInputRef"
            type="file"
            multiple
        ...
        <div v-if="fileItems.length > 0" class="selected-wrap">
          <div class="selected-header">
            <span class="selected-label">{{ lbl('upload_selected_label', 'Selected') }}</span>
            <span class="selected-sub">{{ pendingCount }} {{ lbl('upload_ready_count', 'ready to upload') }}</span>
            <div class="spacer" />
            <v-button
              x-small
              secondary
              :disabled="isUploading"
              @click="clearAll"
              >{{ lbl('upload_clear_all', 'Clear all') }}</v-button
            >
          </div>
        ...
        <p v-if="settings.geo_enabled && !settings.geo_required" class="geo-help">
          {{ lbl('upload_geo_optional', 'Optional. Selected geography IDs will be saved on the uploaded file.') }}
        </p>
      </v-card-text>
      ...
      >
        ...
        {{ lbl('upload_geo_required_error', 'Please fill in required geography fields') }}: {{ missingGeoLevels.map((l) => l.label).join(', ') }}
      </div>
      ...
      <v-card-actions class="card-actions">
        ...
        <v-button
          ...
          @click="startUpload"
        >
          {{ t('upload') }} {{ pendingCount > 0 ? pendingCount : "" }}
        </v-button>
      </v-card-actions>
    </v-card>
  </v-dialog>
