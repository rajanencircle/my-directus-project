...
const geoSearchPlaceholder = computed(() => {
  const template = resolveTranslatable(settings.value.geo_search_placeholder, t, 'Search {label}…')
  return template.replace('{label}', props.label)
})
const geoLoading = computed(() => resolveTranslatable(settings.value.geo_loading, t, 'Loading…'))
const geoNoResults = computed(() => resolveTranslatable(settings.value.geo_no_results, t, 'No results found'))
...
// selects or clears this field — prevents cascade-filled parents from
// immediately wiping out the just-chosen value.
...
async function search(term: string) {
  ...
  try {
    ...
    console.error('[media-library] GeoIndividualSelect search error:', e);
    items.value = [];
  } finally {
    loading.value = false;
  }
...
async function onDrawerInput(val: AnyRecord) {
  ...
  try {
    ...
    console.error(`[media-library] Error creating item in ${props.targetCollection}:`, e);
  }
  drawerOpen.value = false;
}
  ...
  (newFilters, oldFilters) => {
    ...
    for (const f of props.filterBy ?? []) {
      ...
      if (newVal !== oldVal) {
        ...
        // Suppress if we just selected/cleared this field — cascade-filled
        // parents should not immediately wipe out the chosen value
  ...
  <div class="geo-individual-select">
    ...
    <div class="input-wrap" :class="{ 'has-error': showError }">
      ...
      <div v-if="active" class="dropdown">
        ...
        <div v-else class="dropdown-item empty">{{ geoNoResults }}</div>
      </div>
    </div>
  </div>
</template>
...
.input-wrap.has-error :deep(.v-input) {
