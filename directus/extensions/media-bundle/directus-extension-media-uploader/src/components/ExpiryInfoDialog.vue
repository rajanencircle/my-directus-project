  ...
  <v-dialog v-model="open" @update:model-value="(v: boolean) => !v && (open = false)">
    <v-card class="expiry-card">
      <v-card-title class="expiry-title">
        ...
        {{ lbl('expiryTitle', "Don't use expired media") }}
      </v-card-title>

      <v-card-text class="expiry-body">
        <p class="line">
          <strong>{{ displayName }}</strong> {{ lbl('expiryBody', 'has an expiry date') }}
          <strong v-if="formattedDate">({{ formattedDate }})</strong>.
        </p>
        <p class="line muted">
          {{ lbl('expiryWarning', 'Please avoid using this image in content if the expiry date has passed.') }}
        </p>
      </v-card-text>
