<template>
  ...
  >
    <template v-for="level in parsedLevels" :key="level.field">
      <div v-if="!level.hidden" class="cascade-field">
        ...
        <div class="field-input">
          <v-input
            ...
            :placeholder="`Search ${resolveLabel(level)}...`"
            :disabled="disabled"
...
export const DEFAULT_FILTER_MAPPINGS = JSON.stringify(
  {
    place: [{ fk: "country_id", from: "country" }],
  },
  null,
  2,
);
...
interface LevelConfig {
  field: string;
  collection: string;
  /** Plain string OR translation map keyed by language code, e.g. { "en-GB": "Place", "de-DE": "Ort" } */
   ...
   */
  displayField?: string;
  /** When true, this level participates in cascades and data storage but is not rendered in the UI */
  hidden?: boolean;
}
 ...
 */
/**
 * Resolve a level's label to a plain string.
 * - If label is a string → return as-is.
 * - If label is a translation map → try languageCode, then "en-GB", then first value.
 */
function resolveLabel(level: LevelConfig): string {
  const { label } = level;
...
const activeFilterHints = computed<Record<string, string>>(() => {
  ...
  for (const level of parsedLevels.value) {
    ...
    result[level.field] = filters
      .map((f) => {
        const parentLevel = parsedLevels.value.find(
          (l) => l.field === f.fromField,
        );
        const parentLabel = parentLevel
          ? resolveLabel(parentLevel)
          : f.fromField;
