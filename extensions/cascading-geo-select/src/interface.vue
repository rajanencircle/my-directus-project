 ...
 * without having to consult the extension's levels config.
 *
 * Example stored value for the `sadasd` JSON field:
 * {
 *   "place":              { "id": "8cc17e4c-...", "collection": "places" },
 *   "state":              { "id": "7d21669c-...", "collection": "states" },
 *   "region":             { "id": "c2115ea5-...", "collection": "regions_geo" },
 *   "country":            { "id": "8c486bf4-...", "collection": "countries_geo" },
 *   "destination":        { "id": "5a21fcc3-...", "collection": "destinations" },
 *   "destination_cluster":{ "id": "a0aa93a8-...", "collection": "destinations_cluster" }
 * }
 *
 * API usage:
 ...
 * so old records continue to load correctly.
 */
function extractId(val: unknown): string | null {
  if (!val) return null;
  if (typeof val === "string") return val; // legacy bare UUID
...
/**
 * Build the object that gets persisted for a single level.
 * Returns null when there is no selection (clears the field).
 */
...
// ─── Cascade resolution ───────────────────────────────────────────────────────
/**
 * Walk the cascade mappings starting from `fromField` and return a map of
 * fieldName → StoredFieldValue (i.e. { id, collection }) for every downstream
 * level that has a value.  Storing the collection name here is what allows
 * `/items/demo?fields=*,sadasd.place.*` to return actionable data.
 */
async function resolveCascade(
  ...
  for (const mapping of mappings) {
    ...
    // Find the target level so we can embed its collection name
...
async function initFromValues() {
  ...
  for (const level of parsedLevels.value) {
    // extractId handles both legacy bare strings and new { id, collection } objects
...
async function onSelect(level: LevelConfig, item: DropdownItem) {
  ...
  // 1. Set this field's value — store { id, collection } so API consumers
  //    know which table to query without needing the extension's config.
